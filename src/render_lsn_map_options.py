...
DEFAULT_MAP_IMAGE = "data/assets/client-map/north-america-map-ai-web.png"
...
def render_html(input_path: Path, map_image_path: Path, title: str) -> str:
    ...
    width, height = read_png_size(map_image_path)
    ...
    replacements = {
        ...
        "__MAP_IMAGE__": f"data:image/png;base64,{image_data}",
...
def main() -> None:
    ...
    parser.add_argument("--map-image", default=DEFAULT_MAP_IMAGE, help="Path to browser-ready PNG artwork")
    ...
    html = render_html(input_path, map_image_path, args.title)
...
<body>
  ...
  <header class="topbar">
    ...
    <nav class="modebar" aria-label="Visualization modes">
      ...
      <button class="mode-btn" data-mode="badges">Badges</button>
  ...
  <script>
    ...
    const modeLabels = { points: "Points", regions: "Regions", badges: "Badges", heat: "Heat", hybrid: "Hybrid" };
    const format = new Intl.NumberFormat("en-US");
    const regionAnchors = {
      "Canada North": { x: 880, y: 285, radius: 150 },
      "Canada West": { x: 635, y: 500, radius: 118 },
      "Canada Central": { x: 875, y: 555, radius: 112 },
      "Canada East": { x: 1180, y: 610, radius: 120 },
      "Pacific Northwest": { x: 525, y: 735, radius: 86 },
      "West Coast": { x: 500, y: 1050, radius: 92 },
      "Mountain": { x: 700, y: 955, radius: 110 },
      "Midwest": { x: 925, y: 910, radius: 110 },
      "Northeast": { x: 1235, y: 820, radius: 96 },
      "Southeast": { x: 1095, y: 1140, radius: 112 },
      "South Central": { x: 850, y: 1150, radius: 105 },
      "Mexico North": { x: 735, y: 1330, radius: 78 },
      "Mexico West": { x: 680, y: 1435, radius: 68 },
      "Mexico Central": { x: 780, y: 1465, radius: 98 },
      "Mexico South": { x: 940, y: 1580, radius: 92 }
    };
    ...
    const CanvasMarkerLayer = L.Layer.extend({
      ...
      exactPointToCanvas(d) {
        return this.map.latLngToLayerPoint(d.pointLatlng).subtract(this.topLeft);
      },
      draw() {
        const ctx = this.ctx;
        ctx.clearRect(0, 0, this.viewport.x, this.viewport.y);
        if (this.mode === "points") this.drawExactPoints(ctx);
        else if (this.mode === "badges") this.drawRegionFlags(ctx);
      ...
      drawRegionFlags(ctx) {
        ...
        for (const entry of groups) {
          const x = entry.x;
          const y = entry.y;
          const label = `${entry.cc} ${format.format(entry.count)}`;
          const width = Math.max(48, ctx.measureText(label).width + 18);
          const height = 25;
          this.roundRect(ctx, x - width / 2, y - height / 2, width, height, 7);
          ctx.fillStyle = countryColors[entry.cc] || "#3f3f41";
          ctx.fill();
          ctx.lineWidth = 1.5;
          ctx.strokeStyle = "rgba(255,255,255,.92)";
          ctx.stroke();
      ...
      onClick(event) {
        ...
        if (this.mode === "points") {
          ...
          for (const d of this.points) {
            const point = this.map.latLngToLayerPoint(d.pointLatlng);
    ...
    window.__LSN_MAP_PROOF__ = {
      ...
      renderer: "canvas-exact-points-and-region-aggregates",
      visualContract: "branded_overview_not_gis",
      pointPlacement: {
